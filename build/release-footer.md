...
1. Download `Network-Folder-<version>.app.zip` below, unpack it and move
   `Network Folder.app` to `/Applications` (or anywhere else).
...
For the terminal: `bash smb-automount-install-<version>.sh` (`--list`,
`--uninstall`, `--version`).
